...
SeismicSimulator::SeismicSimulator(const SystemParameters& params)
    ...
    std::cout << "Frequencia dominante configurada: " << dominantFrequency << " Hz." << std::endl;

    // Inicializar os fluidos conforme os parametros fornecidos
    ...
    // Inicializacao adicional
    initialize();

    std::cout << "Construtor do SeismicSimulator concluido com sucesso." << std::endl;
}

void SeismicSimulator::initialize() {
    std::cout << "Inicializando vetores de tempo e frequncia." << std::endl;

    //double T = systemParams.getTotalTime();
    double T = 20;
    int N = numPoints;
    double dt = T / (N - 1); // Passo de tempo
    double df = (N - 1) / (N * T); // Passo de discretizacao da frequencia
    double fNyq = 1.0 / (2.0 * dt); // frequencia de Nyquist

    // Inicializar vetores de tempo e frequencia
    timeVector.resize(N);
    ...
    std::cout << "Vetores de tempo e frequencia inicializados." << std::endl;
    ...
    std::cout << "Inicializacao do simulador concluida com " << N << " pontos." << std::endl;
}



void SeismicSimulator::calculateDynamicParameters() {
    std::cout << "Calculando parametros dinamicos." << std::endl;
    ...
    double wd = 4 * M_PI * dominantFrequency; // frequencia angular dominante
    ...
    for (size_t i = 0; i < numFluids; ++i) {
        ...
        // Propriedades fisicas necessarias
        ...
        // Calculo de kappa conforme o MATLAB
        double kappa = kk * (1.0 - std::sqrt(eta / (F * kk * rhof) * 4.0 / wd));

        // Construir a matriz para calculo dos autovalores
...
void SeismicSimulator::calculateFrequencies() {
    // Configuracao das frequencias conforme o MATLAB
    ...
    // Passo de discretizacao da frequencia
    double df = 1.0 / T;

    // Definindo o vetor de frequencias de acordo com o MATLAB
    frequencyVector.resize(N);
    // Calcular o vetor de frequencia conforme o MATLAB
...
void SeismicSimulator::performFFT() {
    // Atualiza o vetor de frequencias
    ...
    for (size_t i = 0; i < fluids.size(); ++i) {
        ...
        // Geracao do sinal de entrada (Ricker Wavelet)
        ...
        if (inputSignal.size() != numPoints) {
            std::cerr << "Erro: tamanho do sinal de entrada nao corresponde a numPoints." << std::endl;
            inputSignal.resize(numPoints, 0.0); // Redimensiona preenchendo com zeros se necessario
        }

        // Verificar se o sinal gerado nao esta zerado
        double maxInputSignal = *std::max_element(inputSignal.begin(), inputSignal.end());
        std::cout << "Maxima amplitude do sinal de entrada: " << maxInputSignal << std::endl;
        if (maxInputSignal < 1e-6) {
            std::cerr << "Aviso: o sinal de entrada para o fluido " << i + 1 << " tem amplitude muito baixa." << std::endl;
            continue; // Pular este fluido, pois o sinal esta muito baixo
        }
        ...
        try {
            ...
            if (!fftProcessor.isInitialized()) {
                throw std::runtime_error("FFTWrapper nao foi inicializado corretamente.");
            }
            ...
            std::cout << "FFT para o fluido " << i + 1 << " concluida." << std::endl;
            ...
            for (int j = 0; j < numPoints; ++j) {
                vfreq[i][j] = fftResult[j]; // Armazena o resultado da FFT no vetor de frequencia
            }

            // Verificar se o resultado da FFT nao esta zerado
            double maxFFT = std::abs(*std::max_element(vfreq[i].begin(), vfreq[i].end(),
                [](const std::complex<double>& a, const std::complex<double>& b) {
                    return std::abs(a) < std::abs(b);
                }));
            std::cout << "Maxima amplitude da FFT para o fluido " << i + 1 << ": " << maxFFT << std::endl;
            if (maxFFT < 1e-6) {
                std::cerr << "Aviso: o resultado da FFT para o fluido " << i + 1 << " tem amplitude muito baixa." << std::endl;
            }

            std::cout << "FFT para o fluido " << i + 1 << " concluida com sucesso." << std::endl;

        } catch (const std::exception& e) {
            std::cerr << "Erro ao realizar a FFT para o fluido " << i + 1 << ": " << e.what() << std::endl;
        }

        // Mensagem de depuracao para verificar se o loop continua
        std::cout << "Continuando para o proximo fluido." << std::endl;
    }
    std::cout << "FFT para todos os fluidos concluida." << std::endl;
}
...
void SeismicSimulator::performInverseFFT() {
    ...
    for (size_t i = 0; i < fluids.size(); ++i) {
        ...
        std::cout << "IFFT para o fluido " << i + 1 << " concluida com sucesso." << std::endl;
    }
    std::cout << "IFFT para todos os fluidos concluida." << std::endl;
}



void SeismicSimulator::normalizeResults() {
    // Calcular o valor maximo de vmax2 e xmax2
    double vmax2 = 0.0;
    double xmax2 = 0.0;

    std::cout << "fluidos: " << fluids.size() <<" e " << vmax2 << std::endl;

    // Encontrar o maximo valor de v e x para normalizacao
    for (size_t i = 0; i < fluids.size(); ++i) {
    ...
    for (size_t i = 0; i < fluids.size(); ++i) {
        if (vmax2 > 0) { // Evitar divisao por zero
            for (size_t j = 0; j < v[i].size(); ++j) {
                v[i][j] /= vmax2; // Normalizar v
            }
        }

        if (xmax2 > 0) { // Evitar divisao por zero
...
void SeismicSimulator::constructFrequencySolution() {
    std::cout << "Construindo a solucao no dominio da frequencia." << std::endl;
    ...
    // Preparar variaveis para calculo
    ...
    // Calcular o vetor de frequencias conforme o MATLAB
    double T = systemParams.getTotalTime();
    double df = 1.0 / T;
    ...
    // Vetor de frequencias angulares
    ...
    for (size_t i = 0; i < fluids.size(); ++i) {
        std::cout << "Construindo solucao para o fluido " << i + 1 << "..." << std::endl;

        for (int j = 0; j < numPoints; ++j) {
            std::complex<double> wj = w[j];

            // Calcular o valor de h[j] com base nas condicoes de wj
            if (wj == wd) {
                h[j] = -1.0i * M_PI / wd
            ...
            double beta = 1.0 / (C * C - M * (lamb + 2.0 * G));
             ...
             // Calculo dos autovalores q1 e q2
            ...
            // Calculo dos parametros xi1 e xi2
            ...
            // Calculo dos parametros b11, b12, b21, b22
            b11[i] = -a1;
            b21[i] = a1 * xi1;
            b12[i] = -a2;
            b22[i] = a2 * xi2;
            ...
            // Calculo de vfreq e wfreq utilizando os autovalores e autovetores
        ...
        std::cout << "Solucao para o fluido " << i + 1 << " construida com sucesso." << std::endl;
    }
}
