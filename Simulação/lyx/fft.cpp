...
void FFTWrapper::initialize() {
    ...
    // Nao chame cleanup no início. Em vez disso, verifique se os recursos estao alocados.
    // Alocacao de memoria para FFT
    in = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * size);
    out = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * size);

    if (!in || !out) {
        std::cerr << "Falha ao alocar memoria para FFTW." << std::endl;
        throw std::runtime_error("Falha ao alocar memoria para FFTW.");
    }

    // Criacao dos planos FFT
...
void FFTWrapper::cleanup() {
    ...
    if (in) {
        ...
        std::cout << "Memoria de entrada FFTW liberada." << std::endl;
    }
    if (out) {
        fftw_free(out);
        out = nullptr; // Evitar uso posterior
        std::cout << "Memoria de saída FFTW liberada." << std::endl;
    }
...
std::vector<std::complex<double>> FFTWrapper::forward(const std::vector<std::complex<double>>& input) {
    if (input.size() != size) {
        throw std::invalid_argument("O tamanho do vetor de entrada nao corresponde ao tamanho da FFT.");
    }
...
// Funcoes auxiliares fftshift e ifftshift
...
std::vector<std::complex<double>> FFTWrapper::ifftshift(const std::vector<std::complex<double>>& input) {
    std::vector<std::complex<double>> output(size);
    int mid = size / 2;
    // Desloca os elementos para a esquerda, para que o centro seja movido para a primeira posicao
    for (int i = 0; i < size; ++i) {
        int shiftedIndex = (i + mid) % size;
        output[i] = input[shiftedIndex];
    }
